...
const EditStore = () => {
  ...
  let detailAddress = document.getElementById(
    "detailAddress"
  ) as HTMLInputElement;
  let storeDescription = document.getElementById(
    "storeDescription"
  ) as HTMLInputElement;
    ...
    fetch("https://qrecode-back.shop/store/info", {
      ...
      .then((data) => {
        ...
        if (data.data) {
          ...
          timeStr.value =
            data.data.storeStart.substring(0, 2) +
            data.data.storeStart.substring(2) +
            "~" +
            data.data.storeEnd.substring(0, 2) +
            data.data.storeEnd.substring(2);
  ...
  const handleComplete = (data: Address) => {
    const add: HTMLInputElement = document.getElementById(
      "roadAddress"
    ) as HTMLInputElement;
    if (add) {
  ...
  const submit = () => {
    ...
    fetch("https://qrecode-back.shop/store/setstore", {
      ...
      body: JSON.stringify({
        ...
        storeStart:
          timeStr.value.substring(0, 2) + ":" + timeStr.value.substring(3, 5),
        storeEnd:
          timeStr.value.substring(6, 8) + ":" + timeStr.value.substring(9, 11),
    ...
    <>
      <div className={style.container}>
        <div className={style.header}>
          ...
          <div onClick={() => submit()} className={style.complete}>
            완료
          </div>
        </div>
        <div className={style.storeName}>
          <span className={style.star}>* </span>상호명
        </div>
        <input
          id="storeName"
          className={style.searchBox2}
          placeholder="가게 이름을 적어주세요"
          minLength={1}
          maxLength={10}
        />
        <div className={style.storeName}>
          <span className={style.star}>* </span>주소
        </div>
        <input
          id="roadAddress"
        ...
        <div className={style.numBox}>
          <div className={style.boxInner1}>
            <div className={style.storeName}>
              <span className={style.star}>* </span>전화번호
            </div>
            <input
              id="phone"
          ...
          <div className={style.boxInner2}>
            <div className={style.storeName}>
              <span className={style.star}>* </span>영업시간
            </div>
            <input
              id="timeStr"
        ...
        <div className={style.storeName}>
          <span className={style.star}>* </span>한줄 설명
        </div>
        <textarea
          id="storeDescription"
          className={style.twolinebox}
          minLength={1}
          maxLength={40}
          placeholder="간단하게 가게를 소개하세요."
        ></textarea>
        <div className={style.forTag}>
          <div className={style.tagInfo}>태그 (최대 3개, 6글자)</div>
          <input
            ref={tag}
            onKeyUpCapture={enterTag}
            className={style.searchBox2}
            minLength={1}
            maxLength={6}
          ...
          <div>
            {hashTag.length === 0 && (
              <span className={style.noHashTag}>ex) #합리적인가격</span>
            )}
            {hashTag.map((hashtag, i) => (
              <span
                key={i}
                className={style.storeHashtag}
                onClick={() => {
                  removeTag(i);
                }}
              >
